...
> 最新提交: `2f85f13`（docs: 完善文档）；状态机重构+按键+上拉+刹车已在 `c09609a` 提交
...
- 串口阻塞已修：fputc 超时 50ms（不死等）+ 打印限频 500ms（否则拖乱 10ms 节拍、速度环失控）
...
- **README / 项目总结**：等功能全部稳定后最后写（简历收尾）

---

## 五、关键坑（已解决）

1. 编码器 A/B 相接反 → 左 A=PA1/B=PA0 方向才对（右轮未验证，见待做）
